...
void detect_and_link_ahf_file()
{
  ...
  sprintf(file_name, "%s%s%s%.3f%s", 
          AHF_INPUT_PATH, AHF_FILE_BASE, ".z", g_head.Redshift, ".AHF_particles"); 
  ...
  if((fp = fopen(file_name, "r")) != NULL)
    ...
    sprintf(file_name, "%s%s%s%.4d%s%.3f%s", 
            AHF_INPUT_PATH, AHF_FILE_BASE, ".", i, ".z", g_head.Redshift, ".AHF_particles");     
	  if((fp = fopen(file_name, "r")) != NULL)
	    {
  ...
  if(ahf_file_num == 1)
    {
    sprintf(file_name, "%s%s%s%.3f%s", 
            AHF_INPUT_PATH, AHF_FILE_BASE, ".z", g_head.Redshift, ".AHF_halos");
    if((fp_ahf_halo[0] = fopen(file_name, "r")) != NULL)
      state("AHF_halos file opened.");
#ifndef NO_AHF_PROFILE      
    sprintf(file_name, "%s%s%s%.3f%s", 
            AHF_INPUT_PATH, AHF_FILE_BASE, ".z", g_head.Redshift, ".AHF_profiles");
    if((fp_ahf_profile[0] = fopen(file_name, "r")) != NULL) 
      state("AHF_profile file opened.");
#endif      
    sprintf(file_name, "%s%s%s%.3f%s", 
            AHF_INPUT_PATH, AHF_FILE_BASE, ".z", g_head.Redshift, ".AHF_particles");
    ...
    sprintf(file_name, "%s%s%s%.3f%s", 
            AHF_INPUT_PATH, AHF_FILE_BASE, ".z", g_head.Redshift, ".AHF_substructure");
  ...
  else
    {
	  for(i=0; i<ahf_file_num; i++)
	    {
      sprintf(file_name, "%s%s%s%.4d%s%.3f%s", 
              AHF_INPUT_PATH, AHF_FILE_BASE, ".", i, ".z", g_head.Redshift, ".AHF_halos");
      if((fp_ahf_halo[i] = fopen(file_name, "r")) != NULL)
        printf("AHF_halos file %d opened.\n", i);   
#ifndef NO_AHF_PROFILE      
      sprintf(file_name, "%s%s%s%.4d%s%.3f%s", 
              AHF_INPUT_PATH, AHF_FILE_BASE, ".", i, ".z", g_head.Redshift, ".AHF_profiles");
      if((fp_ahf_profile[i] = fopen(file_name, "r")) != NULL)
        printf("AHF_profiles file %d opened.\n", i);    
#endif      
      sprintf(file_name, "%s%s%s%.4d%s%.3f%s", 
              AHF_INPUT_PATH, AHF_FILE_BASE, ".", i, ".z", g_head.Redshift, ".AHF_particles");
...
#ifndef NO_AHF_PROFILE    
    effective_rewind(&fp_ahf_profile[i]);       /* rewind but ommit the comment line */	    
#endif    
