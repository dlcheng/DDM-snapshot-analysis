...
/* Total number of dark matter particles used in the simulation */ 
  ...
  decay_freq = (int) (((double) (total_part_num - g_head.Original_num)) / ((double) g_head.Original_num));
  printf("Decay frequency is %d.\n", decay_freq);
#endif
      ...
      for(j=0; j<local_head.Npart[1]; j++)
        {
         ...
         p_part[filled_num + j].envir_flag = 1;               /* marked as environmental particles at first */
	      }
      fread(&block_size, sizeof(block_size), 1, fp_gadget[i]);      
      /* Read the MASS */
...
#ifdef DECAY_DARK_MATTER
  total_mass = 0;
  global_decayed_mass = 0;
  for(j=0; j<total_part_num; j++)
    {
	   total_mass += p_part[j].mass; 
	   if(p_part[j].id > g_head.Original_num - 1)
	     global_decayed_mass += p_part[j].mass;	 
    }		 
  global_frac = global_decayed_mass / total_mass;
  printf("Decayed dark matter fraction is %.4e\n", global_frac);  
#endif   
...
{
  int i;
  double a = 1.0 / (1.0 + g_head.Redshift);
  ...
  for(i=0; i<total_part_num; i++)
    {
     ...
     if(i != temp_id)
       {
#ifdef DECAY_DARK_MATTER		  
        printf("p_part[%u].id = %u,g_head.original_num = %u\n", i, p_part[i].id, g_head.Original_num);
#else
